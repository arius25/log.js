 ...
 * a table named <application>_log in the database
 *
 * @param {Number} level
 * @param {Object} stream
...
var Log = exports = module.exports = function Log(application, level, stream, dbConnection){
...
Log.prototype = {
    ...
    log_db: function(levelStr, args, callback) {
        if (exports[levelStr] <= this.level) {
            ...
            this.dbConn.query('insert into ' + this.application + '_log (date_created,application,level,message) values(?,?,?,?)', [new Date, this.application, levelStr, msg], function(err) {
                callback(err);
            });
        }
    },
